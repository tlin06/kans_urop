...
def get_file(mode='a'):
    return open('data/timing_10-40_tests_log.txt', mode)
...
time_keys = ['initiate MLP', 'sampling', "generate eloc distr", 'convert to tensor', 
             'generate input samples', 'forward pass', 'calc loss', 
             'loss backward', 'optimizer step']

n_values = [10, 20, 30, 40]; h_values = [0.1, 1, 10]
J = 1
num_epochs = 1000
num_trials = 3
num_samples = 512
...
for N in n_values:
    for Gamma in h_values:
        ...
        for trial in range(num_trials):
            ...
            times['initiate MLP'] += time.time() - start

            for epoch in range(num_epochs):
                start_epoch = time.time()

                start = time.time()
                mh_state = MHNeuralState(N, mlp_model, utils.log_amp_phase, 'idk', torch.zeros(chains, dtype=torch.long), num_samples, burnin = 5*N, lag = N, chains = chains)
                times['sampling'] += time.time() - start

                start = time.time()
                eloc_distr = tfim_functions.generate_eloc_distr(mh_state, N, J, Gamma, mlp_model)
            ...
            torch.save(mlp_model.state_dict(), f'models/n_{N}_h_{Gamma}_GSE_{round(energy.item(), 3)}_t_{trial}.pt')
            all_data.to_csv('data/MLP_n_10-40_data.csv')
