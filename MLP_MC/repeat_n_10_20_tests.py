...
def get_file(mode='a'):
    return open('data/n_10_20_tests_log.txt', mode)
    ...
    for Gamma in h_values:
        ...
        for trial in range(num_trials):
            ...
            for epoch in range(num_epochs):
                ...
                eloc_distr = generate_eloc_distr(mh_state, N, J, Gamma, mlp_model)
                eloc_list = torch.tensor([eloc_distr[x] for x in mh_state.list])
                psi = mlp_model(generate_input_samples(N, mh_state.list))
        ...
        plt.savefig(f'plots/correct_grad_tests/loss_MLP_N_{N}_h_10_{round(np.log10(Gamma), 2)}_samples_{num_samples}.png')
        plt.clf()
        ...
        plt.yscale('log')
        plt.savefig(f'plots/correct_grad_tests/error_MLP_N_{N}_h_10_{round(np.log10(Gamma), 2)}_samples_{num_samples}.png')
        plt.clf()
        f.close()
